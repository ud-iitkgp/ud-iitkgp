
# Utkarsh Dwivedi — Product & AI👋

## About
I build product-led solutions that combine generative AI, data engineering, and domain expertise to deliver measurable business outcomes. My work spans CRM & customer experience platforms, real-time omnichannel systems, B2B relationship CRMs, and ML-driven recommendation system.

---

## Impact Highlights
- **49.4%** – GWP growth (CC Retention)  
- **72.6%** – Advisor productivity uplift  
- **$250M+** – Annual sales uplift (Bancassurance)  
- **44.23%** – PSP growth

---

## Selected Product Work

### 📌 Call Centre Retention — CRM & CX
- Unified Claims, Communication & Servicing into a 360° advisor platform.
- Golden-record clustering & HNI prioritization.
- Delivered *49.4%* ⬆️ GWP growth, *72.6%* ⬆️ productivity uplift, and *8.5%* ⬇️ reduction in tele-caller headcount YoY through data-driven campaign management and near real-time Kafka-based payment sync.

###📌 Call Centre Fresh — Real-Time Omnichannel CRM
- Real-time voice/video/chat platform.
- LLM-driven sales scripts, abandoned-queue recovery & funnel prioritization.
- Forecasted at *43–62%* YoY growth 📈 through a phased rollout (stabilization and scale-up), optimizing conversion and advisor utilization.

###📌 Wholesale & Bancassurance CRM
- Mobile-first RM tool with relationship health scoring.
- Automated cross-sell intelligence & claims-to-sales workflows.
- Resulted in *2.8%* ⬆️ wallet share increase and *$250M+* annual sales uplift through cross-sell, claims-to-sales automation, and improved RM efficiency.
---

## Tech & Methods
- Product Strategy  
- Roadmap & GTM  
- LLM / Generative AI    
- Kafka & Real-time Systems  
- Agile, OKRs  
---
## Publications & Thesis
**M.Tech Thesis — IIT Kharagpur**  
*Detection of Potential Fishing Zones along the Eastern Coast of India using Machine Learning*  
Thermal-front extraction (CAA), Fish Swarm Optimization & ML classification.
---
## Contact
Open to product leadership roles, AI product strategy, B2B/B2C PM roles & research collaborations.
---
## Quick Facts
- IIT Kharagpur 
- ICICI Lombard — Product  
- 3+ Product Launches
- Based out of Mumbai 🏙️
  

<!--
**ud-iitkgp/ud-iitkgp** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
